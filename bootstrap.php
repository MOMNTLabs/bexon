...
function ensureWorkspaceSchema(PDO $pdo): void
{
    ...
    if (tableHasColumn($pdo, 'tasks', 'completed_at')) {
        $pdo->exec(
...
{
    $needsBackfill = false;
    $backfillMetaKey = 'task_extended_backfill_v6';
    ...
    if (!tableHasColumn($pdo, 'tasks', 'completed_at')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE tasks ADD COLUMN completed_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NULL");
        } else {
            $pdo->exec("ALTER TABLE tasks ADD COLUMN completed_at TEXT DEFAULT NULL");
        }
        $needsBackfill = true;
    }
    ...
    $stmt = $pdo->query('SELECT id, workspace_id, status, updated_at, completed_at, assigned_to, group_name, assignee_ids_json, reference_links_json, reference_images_json, review_file_json, subtasks_json, subtasks_dependency_enabled, title_tag FROM tasks');
    ...
    foreach ($rows as $row) {
        ...
        $status = normalizeTaskStatus((string) ($row['status'] ?? 'todo'), $workspaceId > 0 ? $workspaceId : null);
...
function ensureWorkspaceAccountingSchema(PDO $pdo): void
{
    if (dbDriverName($pdo) === 'pgsql') {
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS workspace_accounting_periods (
                ...
                balance_snapshot_cents BIGINT DEFAULT NULL,
                balance_snapshot_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NULL,
                updated_by BIGINT DEFAULT NULL REFERENCES users(id) ON DELETE SET NULL,
                updated_at TIMESTAMP WITHOUT TIME ZONE NOT NULL
            )'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS workspace_accounting_periods (
                ...
                balance_snapshot_cents INTEGER DEFAULT NULL,
                balance_snapshot_at TEXT DEFAULT NULL,
                updated_by INTEGER DEFAULT NULL,
                updated_at TEXT NOT NULL,
                FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE,
                FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL
    ...
    if (!tableHasColumn($pdo, 'workspace_accounting_entries', 'settled_at')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN settled_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NULL");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN settled_at TEXT DEFAULT NULL");
        }
    }
    ...
    if (!tableHasColumn($pdo, 'workspace_accounting_entries', 'automation_type')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN automation_type VARCHAR(24) NOT NULL DEFAULT 'manual'");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN automation_type TEXT NOT NULL DEFAULT 'manual'");
        }
    }
    if (!tableHasColumn($pdo, 'workspace_accounting_entries', 'task_link_workspace_id')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN task_link_workspace_id BIGINT DEFAULT NULL");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN task_link_workspace_id INTEGER DEFAULT NULL");
        }
    }
    ...
    if (!tableHasColumn($pdo, 'workspace_accounting_entries', 'task_link_rate_cents')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN task_link_rate_cents BIGINT NOT NULL DEFAULT 0");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_entries ADD COLUMN task_link_rate_cents INTEGER NOT NULL DEFAULT 0");
        }
    }
    ...
    if (!tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_cents')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_periods ADD COLUMN balance_snapshot_cents BIGINT DEFAULT NULL");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_periods ADD COLUMN balance_snapshot_cents INTEGER DEFAULT NULL");
        }
    }
    if (!tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_at')) {
        if (dbDriverName($pdo) === 'pgsql') {
            $pdo->exec("ALTER TABLE workspace_accounting_periods ADD COLUMN balance_snapshot_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NULL");
        } else {
            $pdo->exec("ALTER TABLE workspace_accounting_periods ADD COLUMN balance_snapshot_at TEXT DEFAULT NULL");
        }
    }
    ...
    if ($rows) {
        ...
        foreach ($rows as $row) {
            ...
            $normalizedSettledAt = $normalizedSettled === 1
                ? (
                    accountingDateTimeForStorage($row['settled_at'] ?? null)
                    ?? accountingDateTimeForStorage($row['updated_at'] ?? null)
                    ?? accountingDateTimeForStorage($row['created_at'] ?? null)
                    ?? nowIso()
                )
                : null;
            ...
            $normalizeStmt->execute([
                ...
                ':settled_at' => $normalizedSettledAt,
        ...
        $periodNormalizeStmt = $pdo->prepare(
            'UPDATE workspace_accounting_periods
             SET period_key = :period_key,
                 ...
                 balance_snapshot_cents = :balance_snapshot_cents,
                 balance_snapshot_at = :balance_snapshot_at,
                 updated_at = :updated_at
             WHERE id = :id'
        );
        foreach ($periodRows as $periodRow) {
            $normalizedBalanceSnapshotAt = accountingDateTimeForStorage($periodRow['balance_snapshot_at'] ?? null);
            $normalizedBalanceSnapshotCents = $normalizedBalanceSnapshotAt !== null
                ? (normalizeSignedDueAmountCents($periodRow['balance_snapshot_cents'] ?? null) ?? 0)
                : null;
            $periodNormalizeStmt->execute([
                ...
                ':balance_snapshot_cents' => $normalizedBalanceSnapshotCents,
                ':balance_snapshot_at' => $normalizedBalanceSnapshotAt,
...
function workspaceAccountingSchemaCapabilities(PDO $pdo): array
{
    ...
    $capabilities = [
        ...
        'settled_at' => tableHasColumn($pdo, 'workspace_accounting_entries', 'settled_at'),
    ];
    ...
    if (
        ...
        $capabilities['settled_at'] = tableHasColumn($pdo, 'workspace_accounting_entries', 'settled_at');
    }
...
function workspaceAccountingPeriodSchemaCapabilities(PDO $pdo): array
{
    static $cache = [];
    ...
    $capabilities = [
        'balance_snapshot_cents' => tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_cents'),
        'balance_snapshot_at' => tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_at'),
    ];

    if (
        !$capabilities['balance_snapshot_cents']
        || !$capabilities['balance_snapshot_at']
    ) {
        try {
            ensureWorkspaceAccountingSchema($pdo);
        } catch (Throwable $_) {
        ...
        $capabilities['balance_snapshot_cents'] = tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_cents');
        $capabilities['balance_snapshot_at'] = tableHasColumn($pdo, 'workspace_accounting_periods', 'balance_snapshot_at');
    }
...
function accountingDateTimeForStorage($dateTimeValue): ?string
{
    $dateTimeValue = trim((string) $dateTimeValue);
    if ($dateTimeValue === '') {
        return null;
    ...
    $dateOnlyValue = dueDateForStorage($dateTimeValue);
    if ($dateOnlyValue !== null && strlen($dateTimeValue) <= 10) {
        return $dateOnlyValue . ' 00:00:00';
    }
...
function accountingDateTimeIsAfter($candidateValue, $anchorValue): bool
{
    $candidate = accountingDateTimeForStorage($candidateValue);
    $anchor = accountingDateTimeForStorage($anchorValue);
    if ($candidate === null || $anchor === null) {
        return false;
    }

    return strcmp($candidate, $anchor) > 0;
}
...
function resolveAccountingSettledAtValue(int $settledFlag, $existingSettledAt = null, $fallbackTimestamp = null): ?string
{
    if ($settledFlag !== 1) {
        return null;
    }

    $resolvedExistingSettledAt = accountingDateTimeForStorage($existingSettledAt);
    if ($resolvedExistingSettledAt !== null) {
        return $resolvedExistingSettledAt;
    }

    return accountingDateTimeForStorage($fallbackTimestamp) ?? nowIso();
}
...
function normalizeAccountingAutomationType(string $value): string
{
    return mb_strtolower(trim($value)) === 'completed_tasks'
        ? 'completed_tasks'
        : 'manual';
}
...
function workspaceAccountingNormalizeEntryRow(array $row, string $defaultPeriodKey): array
{
    ...
    $row['installment_progress'] = $row['is_installment'] === 1
        ...
        : '';
    $row['automation_type'] = normalizeAccountingAutomationType((string) ($row['automation_type'] ?? 'manual'));
    $taskLinkWorkspaceId = isset($row['task_link_workspace_id']) ? (int) ($row['task_link_workspace_id'] ?? 0) : 0;
    $row['task_link_workspace_id'] = $taskLinkWorkspaceId > 0 ? $taskLinkWorkspaceId : null;
    $taskLinkGroupName = trim((string) ($row['task_link_group_name'] ?? ''));
    ...
    $row['task_link_rate_input'] = dueAmountLabelFromCents($row['task_link_rate_cents']);
    $row['is_task_linked'] = (
...
function accountingPeriodRangeBounds(string $periodKey): array
{
    $normalizedPeriodKey = normalizeAccountingPeriodKey($periodKey);
    $periodDate = DateTimeImmutable::createFromFormat('!Y-m', $normalizedPeriodKey)
        ?: new DateTimeImmutable('first day of this month');

    return [
        'start' => $periodDate->format('Y-m-01'),
        'next_start' => $periodDate->modify('+1 month')->format('Y-m-01'),
    ];
}

function workspaceCompletedTasksForAccountingTaskLink(PDO $pdo, int $workspaceId, string $periodKey): array
{
    static $cache = [];
    ...
    $cacheKey = spl_object_id($pdo) . ':' . $workspaceId . ':' . normalizeAccountingPeriodKey($periodKey);
    if (isset($cache[$cacheKey])) {
        return $cache[$cacheKey];
    }

    $range = accountingPeriodRangeBounds($periodKey);
    $stmt = $pdo->prepare(
        'SELECT id,
                group_name,
                assigned_to,
         ...
         WHERE workspace_id = :workspace_id
           ...
           AND completed_at < :next_period_start'
    );
    $stmt->execute([
        ':workspace_id' => $workspaceId,
        ':period_start' => $range['start'],
        ':next_period_start' => $range['next_start'],
    ]);
    $rows = $stmt->fetchAll() ?: [];
    $usersById = usersMapById($workspaceId);

    $cache[$cacheKey] = array_map(
        static function (array $row) use ($usersById): array {
            return [
                'id' => (int) ($row['id'] ?? 0),
                'group_name' => normalizeTaskGroupName((string) ($row['group_name'] ?? 'Geral')),
                'assignee_ids' => normalizeAssigneeIds(
                    decodeAssigneeIds(
                        $row['assignee_ids_json'] ?? null,
                        isset($row['assigned_to']) ? (int) ($row['assigned_to'] ?? 0) : null
                    ),
                    $usersById
                ),
...
function workspaceAccountingApplyCompletedTaskAutomation(PDO $pdo, array $entry): array
{
    ...
    $selectedAssigneeLookup = array_fill_keys($selectedAssigneeIds, true);
    $matchingTaskCount = 0;
    ...
    foreach (workspaceCompletedTasksForAccountingTaskLink($pdo, $sourceWorkspaceId, $periodKey) as $task) {
        ...
        if ($selectedAssigneeLookup) {
            $taskAssigneeIds = is_array($task['assignee_ids'] ?? null) ? $task['assignee_ids'] : [];
            $hasMatchingAssignee = false;
            foreach ($taskAssigneeIds as $taskAssigneeId) {
                if (isset($selectedAssigneeLookup[(int) $taskAssigneeId])) {
    ...
    $rateCents = max(0, (int) ($entry['task_link_rate_cents'] ?? 0));
    $computedAmountCents = $matchingTaskCount * $rateCents;
    $entry['task_link_match_count'] = $matchingTaskCount;
    $entry['amount_cents'] = $computedAmountCents;
    $entry['total_amount_cents'] = $computedAmountCents;
    $entry['amount_display'] = dueAmountLabelFromCents($computedAmountCents);
    $entry['amount_input'] = dueAmountLabelFromCents($computedAmountCents);
    $entry['total_amount_display'] = dueAmountLabelFromCents($computedAmountCents);
    $entry['total_amount_input'] = dueAmountLabelFromCents($computedAmountCents);

    static $workspaceNames = [];
    if (!isset($workspaceNames[$sourceWorkspaceId])) {
    ...
    foreach ($selectedAssigneeIds as $assigneeId) {
        if (!isset($sourceUsersById[$assigneeId])) {
            continue;
        }
        $taskLinkAssigneeNames[] = normalizeUserDisplayName((string) ($sourceUsersById[$assigneeId]['name'] ?? ''));
    }
    $entry['task_link_assignee_names'] = array_values(array_filter($taskLinkAssigneeNames));
    $entry['task_link_assignee_summary'] = $entry['task_link_assignee_names']
        ? implode(', ', $entry['task_link_assignee_names'])
        : 'Todos os responsáveis';
    $entry['task_link_summary_label'] = $matchingTaskCount . ' concluída' . ($matchingTaskCount === 1 ? '' : 's')
        . ' x '
        . dueAmountLabelFromCents($rateCents);
    $entry['task_link_scope_label'] = trim(
        implode(
            ' • ',
            array_values(
                array_filter([
...
function workspaceAccountingApplyAutomations(PDO $pdo, array $entries): array
{
    if (!$entries) {
        return $entries;
    }
...
function accountingTaskLinkOptionsForUser(int $userId): array
{
    if ($userId <= 0) {
        return [
            'workspaces' => [],
    ...
    foreach (workspaceMembershipsDetailedForUser($userId) as $workspace) {
        $workspaceId = (int) ($workspace['id'] ?? 0);
        if ($workspaceId <= 0 || !userHasWorkspaceAccess($userId, $workspaceId)) {
            continue;
        }

        $workspaces[] = [
            'id' => $workspaceId,
            'name' => normalizeWorkspaceName((string) ($workspace['name'] ?? 'Workspace')),
        ];

        $visibleGroups = [];
        foreach (taskGroupsList($workspaceId) as $groupName) {
            if (!userCanViewTaskGroup($userId, $workspaceId, $groupName)) {
                continue;
            }
            $visibleGroups[] = normalizeTaskGroupName((string) $groupName);
        }
        $groupsByWorkspace[(string) $workspaceId] = array_values(array_unique($visibleGroups));

        $workspaceUsers = [];
        foreach (usersList($workspaceId) as $workspaceUser) {
            ...
            $workspaceUsers[] = [
                'id' => $workspaceUserId,
                'name' => normalizeUserDisplayName((string) ($workspaceUser['name'] ?? 'Usuário')),
                'avatar' => userAvatarImageSrc($workspaceUser),
                'initial' => userDisplayInitial((string) ($workspaceUser['name'] ?? 'U')),
            ];
        }
        $usersByWorkspace[(string) $workspaceId] = $workspaceUsers;
    }

    return [
        'workspaces' => $workspaces,
        'groups_by_workspace' => $groupsByWorkspace,
        'users_by_workspace' => $usersByWorkspace,
    ];
}

function workspaceAccountingResolvedAutomationConfig(
    PDO $pdo,
    string $periodKey,
    string $entryType,
    ...
    $resolved = [
        ...
        'task_link_match_count' => 0,
        'computed_amount_cents' => null,
    ];

    $automationType = normalizeAccountingAutomationType((string) ($automationConfig['automation_type'] ?? 'manual'));
    if ($automationType !== 'completed_tasks') {
        return $resolved;
    }
    ...
    $sourceWorkspaceId = max(0, (int) ($automationConfig['task_link_workspace_id'] ?? 0));
    ...
    $selectedAssigneeIds = normalizeAssigneeIds(
        is_array($automationConfig['task_link_assignee_ids'] ?? null) ? $automationConfig['task_link_assignee_ids'] : []
    );
    $rateCents = normalizeDueAmountCents($automationConfig['task_link_rate_cents'] ?? null);
    if ($rateCents === null || $rateCents <= 0) {
        throw new RuntimeException('Informe um valor válido por tarefa concluída.');
    }

    $previewEntry = workspaceAccountingApplyCompletedTaskAutomation($pdo, [
        'is_task_linked' => 1,
    ...
    $resolved['task_link_match_count'] = max(0, (int) ($previewEntry['task_link_match_count'] ?? 0));
    $resolved['computed_amount_cents'] = max(0, (int) ($previewEntry['amount_cents'] ?? 0));
...
function workspaceAccountingCreateDueLinkedEntry(PDO $pdo, array $payload, int $isSettled = 0): int
{
    ...
    $createdAt = nowIso();
    $settledAt = resolveAccountingSettledAtValue($isSettled === 1 ? 1 : 0, null, $createdAt);
...
function workspaceAccountingUpdateDueLinkedEntry(
    ...
    array $payload,
    int $isSettled = 0,
    ?string $existingSettledAt = null
): void {
    $settledAt = resolveAccountingSettledAtValue($isSettled === 1 ? 1 : 0, $existingSettledAt);
    $stmt = $pdo->prepare(
        'UPDATE workspace_accounting_entries
         SET period_key = :period_key,
             entry_type = :entry_type,
...
function workspaceAccountingEnsureMonthlyDueEntry(
    ...
    workspaceAccountingUpdateDueLinkedEntry(
        $pdo,
        $workspaceId,
        (int) ($existingEntry['id'] ?? 0),
        $payload,
        $settledFlag,
        (string) ($existingEntry['settled_at'] ?? '')
    );
...
function workspaceAccountingBalanceSnapshot(?int $workspaceId = null, ?string $periodKey = null): ?array
{
    $workspaceId = $workspaceId && $workspaceId > 0 ? $workspaceId : activeWorkspaceId();
    if ($workspaceId === null) {
        return null;
    ...
    if (
        empty($periodSchema['balance_snapshot_cents'])
        || empty($periodSchema['balance_snapshot_at'])
    ) {
        return null;
    }

    $stmt = $pdo->prepare(
        'SELECT balance_snapshot_cents, balance_snapshot_at
         FROM workspace_accounting_periods
         WHERE workspace_id = :workspace_id
           AND period_key = :period_key
         LIMIT 1'
    ...
    $snapshotAt = accountingDateTimeForStorage($row['balance_snapshot_at'] ?? null);
    if ($snapshotAt === null) {
        return null;
    }

    $snapshotCents = normalizeSignedDueAmountCents($row['balance_snapshot_cents'] ?? null) ?? 0;

    return [
        'amount_cents' => $snapshotCents,
        'amount_display' => dueAmountLabelFromSignedCents($snapshotCents),
        'snapshot_at' => $snapshotAt,
        'snapshot_at_display' => accountingDateTimeLabel($snapshotAt),
    ];
}
...
function workspaceAccountingOpeningBalanceCents(?int $workspaceId = null, ?string $periodKey = null): int
{
    ...
    while ($cursor < $periodKey) {
        ...
        $periodSummary = accountingSummary(
            $periodEntries,
            $openingBalance,
            $cursor === $currentPeriodKey && $currentPeriodSnapshot !== null
                ? [
                    'period_key' => $cursor,
                    'current_period_key' => $currentPeriodKey,
                    'balance_snapshot_cents' => (int) ($currentPeriodSnapshot['amount_cents'] ?? 0),
                    'balance_snapshot_at' => (string) ($currentPeriodSnapshot['snapshot_at'] ?? ''),
                ]
                : []
        );
...
function setWorkspaceAccountingBalanceSnapshot(
    PDO $pdo,
    int $workspaceId,
    ?string $periodKey,
    $amountInput,
    ...
    $periodKey = normalizeAccountingPeriodKey($periodKey);
    workspaceAccountingPeriodSchemaCapabilities($pdo);
    ...
    $amountCents = ($rawAmountInput === '' || $rawAmountInput === null)
        ...
        : normalizeSignedDueAmountCents($amountInput);
    if ($amountCents === null) {
        throw new RuntimeException('Informe um saldo conciliado vÃ¡lido.');
    }

    $resolvedSnapshotAt = accountingDateTimeForStorage($snapshotAt) ?? nowIso();
    $updatedAt = nowIso();
    ...
    if (dbDriverName($pdo) === 'pgsql') {
        $stmt = $pdo->prepare(
            ...
            ON CONFLICT (workspace_id, period_key)
            DO UPDATE SET
                balance_snapshot_cents = EXCLUDED.balance_snapshot_cents,
                balance_snapshot_at = EXCLUDED.balance_snapshot_at,
                updated_by = EXCLUDED.updated_by,
                updated_at = EXCLUDED.updated_at'
        );
    } else {
    ...
    $stmt->bindValue(':workspace_id', $workspaceId, PDO::PARAM_INT);
    $stmt->bindValue(':period_key', $periodKey, PDO::PARAM_STR);
    $stmt->bindValue(':balance_snapshot_cents', $amountCents, PDO::PARAM_INT);
    $stmt->bindValue(':balance_snapshot_at', $resolvedSnapshotAt, PDO::PARAM_STR);
    if ($updatedBy !== null && $updatedBy > 0) {
        $stmt->bindValue(':updated_by', $updatedBy, PDO::PARAM_INT);
    } else {
        $stmt->bindValue(':updated_by', null, PDO::PARAM_NULL);
    ...
    return [
        'amount_cents' => $amountCents,
        'amount_display' => dueAmountLabelFromSignedCents($amountCents),
        'snapshot_at' => $resolvedSnapshotAt,
        'snapshot_at_display' => accountingDateTimeLabel($resolvedSnapshotAt),
    ];
}
...
function createWorkspaceAccountingEntry(
    ...
    $resolvedAutomation = workspaceAccountingResolvedAutomationConfig($pdo, $periodKey, $entryType, $automationConfig);
    $isCompletedTasksAutomation = $resolvedAutomation['automation_type'] === 'completed_tasks';
    $requestedMonthlyMode = normalizeAccountingMonthlyMode(
        (string) ($monthlyMode ?? 'uniform'),
        $entryType,
        $isMonthly === 1 ? 1 : 0
    );
    if ($isCompletedTasksAutomation) {
        $isInstallment = 0;
        $isMonthly = 0;
    ...
    if ($isCompletedTasksAutomation) {
        $computedAmountCents = max(0, (int) ($resolvedAutomation['computed_amount_cents'] ?? 0));
        $amountPayload = [
            'amount_cents' => $computedAmountCents,
            'total_amount_cents' => $computedAmountCents,
            'is_installment' => 0,
            'installment_number' => 0,
            'installment_total' => 0,
        ];
    ...
    $createdAt = nowIso();
    $settledAt = resolveAccountingSettledAtValue($settledFlag, null, $createdAt);
    ...
    $stmt->bindValue(':paid_amount_cents', $paidAmountCents, PDO::PARAM_INT);
    $stmt->bindValue(':automation_type', (string) ($resolvedAutomation['automation_type'] ?? 'manual'), PDO::PARAM_STR);
    if (($resolvedAutomation['task_link_workspace_id'] ?? null) !== null) {
        $stmt->bindValue(':task_link_workspace_id', (int) $resolvedAutomation['task_link_workspace_id'], PDO::PARAM_INT);
    } else {
        $stmt->bindValue(':task_link_workspace_id', null, PDO::PARAM_NULL);
    }
    if (($resolvedAutomation['task_link_group_name'] ?? null) !== null) {
        $stmt->bindValue(':task_link_group_name', (string) $resolvedAutomation['task_link_group_name'], PDO::PARAM_STR);
    } else {
...
function updateWorkspaceAccountingEntry(
    ...
    $settledAt = resolveAccountingSettledAtValue($settledFlag, $existingSettledAt);
    ...
    $stmt->execute([
        ...
        ':paid_amount_cents' => $paidAmountCents,
        ':automation_type' => (string) ($resolvedAutomation['automation_type'] ?? 'manual'),
        ':task_link_workspace_id' => $resolvedAutomation['task_link_workspace_id'] !== null
...
function workspaceAccountingEntryMovementTimestamp(array $entry): ?string
{
    if (((int) ($entry['is_monthly_goal'] ?? 0)) === 1) {
        return null;
    }

    if (((int) ($entry['is_auto_received'] ?? 0)) === 1) {
        return accountingDateTimeForStorage((string) ($entry['due_date'] ?? ''));
    }
    ...
    return accountingDateTimeForStorage($entry['settled_at'] ?? null)
        ?? accountingDateTimeForStorage($entry['updated_at'] ?? null)
        ?? accountingDateTimeForStorage($entry['created_at'] ?? null);
}

function accountingSnapshotMovementDeltaCents(array $entries, string $snapshotAt): int
{
    $snapshotAt = accountingDateTimeForStorage($snapshotAt) ?? '';
    if ($snapshotAt === '') {
        return 0;
    }
    ...
    foreach ($entries as $entry) {
        $entryType = normalizeAccountingEntryType((string) ($entry['entry_type'] ?? 'expense'));
        if (((int) ($entry['is_monthly_goal'] ?? 0)) === 1) {
            $goalPaymentHistory = is_array($entry['goal_payment_history'] ?? null)
                ? $entry['goal_payment_history']
                : [];
            foreach ($goalPaymentHistory as $goalPaymentHistoryItem) {
                if (!accountingDateTimeIsAfter($goalPaymentHistoryItem['created_at'] ?? null, $snapshotAt)) {
                    continue;
                }

                $deltaCents -= max(0, normalizeDueAmountCents($goalPaymentHistoryItem['amount_cents'] ?? null) ?? 0);
            }
            continue;
        }
        if (((int) ($entry['is_settled'] ?? 0)) !== 1) {
        ...
        $movementTimestamp = workspaceAccountingEntryMovementTimestamp($entry);
        if ($movementTimestamp === null || !accountingDateTimeIsAfter($movementTimestamp, $snapshotAt)) {
            continue;
        }
...
function accountingSummary(array $entries, int $openingBalanceCents, array $options = []): array
{
    $expenseTotal = 0;
    $expensePaid = 0;
    $expenseDiscount = 0;
...
function accountingWeeklyBalanceProjection(
    ...
    foreach ($weeks as &$week) {
        ...
        $week['opening_balance_cents'] = $balanceCents;
        $week['opening_balance_display'] = dueAmountLabelFromSignedCents($balanceCents);
...
function workspaceAccountingNextIncomeProjectionSummary(
    ...
    $currentSummary = accountingSummary($currentPeriodEntries, $openingBalanceCents, $summaryOptions);
...
function workspaceUpdateTaskStatusConfiguration(
    ...
    if ($removeStatusKey !== '') {
        ...
        if ($removeIndex !== null && $removeIndex > 0 && $removeIndex < count($workingList) - 1) {
            ...
            $fallbackIsDone = taskStatusKind($fallbackKey, $workspaceId) === 'done';
            $remapStmt = $pdo->prepare(
                'UPDATE tasks
                 SET status = :fallback_status,
                     completed_at = ' . ($fallbackIsDone ? 'COALESCE(completed_at, :completed_at)' : 'NULL') . '
                 WHERE workspace_id = :workspace_id
            ...
            if ($fallbackIsDone) {
                $remapParams[':completed_at'] = nowIso();
            }
            $remapStmt->execute($remapParams);
...
function taskCompletedAtValueForStatusChange(
    string $status,
    ?string $previousStatus = null,
    ?string $existingCompletedAt = null,
    ?string $transitionedAt = null,
    ?int $workspaceId = null,
    ?array $workspace = null
): ?string {
    $currentCompletedAt = trim((string) ($existingCompletedAt ?? ''));
    $currentCompletedAt = $currentCompletedAt !== '' ? $currentCompletedAt : null;
    $nextIsDone = taskStatusKind($status, $workspaceId, $workspace) === 'done';
    if (!$nextIsDone) {
        return null;
    }

    $previousIsDone = $previousStatus !== null
        && taskStatusKind($previousStatus, $workspaceId, $workspace) === 'done';
    if ($previousIsDone && $currentCompletedAt !== null) {
        return $currentCompletedAt;
    }

    $resolvedTransitionedAt = trim((string) ($transitionedAt ?? ''));
    return $resolvedTransitionedAt !== '' ? $resolvedTransitionedAt : nowIso();
}
